...
class ThemifyBuilder_Data_Manager {
	...
	public function __construct() {
		...
		if ( current_user_can( 'manage_options' ) ) {
			add_action( 'admin_init', array( __CLASS__, 'regenerate_notice' ) );
		}
		if ( is_multisite() ) {
	 ...
	 */
	public static function regenerate_notice() {
		// checks if need to regenerate show the notice
	    if ( self::needs_regenerate() ) {
			if( (!empty($_GET['tb_regenerate_files']) && 'finished' === Themify_Builder::regenerate_css_files() )) {
				update_option('tb_css_version',self::$latest_CSS_version);
			}else{
				add_action( 'admin_notices', array( __CLASS__, 'make_regenerate_notice' ) );
			}
		} else {
	...
	public static function needs_regenerate() {
		...
		if(false === $css_version){
			...
			$result = $wp_filesystem->execute->exists( $path );
		}else{
			$result = $css_version !== self::$latest_CSS_version;
		}
		return $result;
	}

	/**
	 * Produce a notice to regenerate CSS files base on new version
	 */
	public static function make_regenerate_notice() {
		$url = '//'.$_SERVER["HTTP_HOST"] . $_SERVER["REQUEST_URI"];
		$url .= empty($_SERVER["QUERY_STRING"]) ? '?' : '&' ;
		$url .= 'tb_regenerate_files=true';
	    ?>
        <div class="notice notice-warning is-dismissible">
            <p>
                <strong><?php _e( 'Themify Builder: We need to regenerate your Builder layout CSS files.', 'themify' ); ?></strong>
                <a href="<?php echo $url; ?>" class="button button-primary"><?php _e( 'Regenerate CSS', 'themify' ); ?></a>
            </p>
        </div>
	    <?php
	}
